...
const HODDashboard: React.FC<HODDashboardProps> = ({
  ...
  React.useEffect(() => {
    if (!selectedEventId && eventsList?.[0]?.id) {
  ...
  const sectionParticipationCount = React.useMemo(() => {
    if (!selectedEvent) return 0;
    return odRequests
      .filter((req) => {
        const reqEventId = (req as any).eventId as string | undefined;
        const reqEventName = (req as any).eventName as string | undefined;
        return (
          (reqEventId && reqEventId === selectedEvent.id) ||
          (reqEventName && reqEventName === selectedEvent.title)
        );
      })
      .filter((req) => {
        if (!faSection) return true;
        const reqSection = (req as any).section as string | undefined;
        return reqSection === faSection;
      }).length;
  }, [odRequests, selectedEvent, faSection]);
    ...
    <div className="animate-in fade-in duration-700 pb-20">
      <div className="mb-10 flex flex-col md:flex-row justify-between items-end gap-6">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => setView("fa-od-approvals")}
            className="bg-slate-900 text-white px-6 py-3 rounded-2xl font-bold text-sm flex items-center gap-2 hover:bg-slate-800 transition-all shadow-xl shadow-slate-200 active:scale-95"
          >
          ...
          <div className="bg-[#1e293b] rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl h-full flex flex-col justify-center">
            ...
            <div className="space-y-8 relative z-10">
              ...
              <div className="pt-8 border-t border-white/10">
                ...
                <p className="font-black text-lg font-mono tracking-wider">
                  {HOD_PROFILE.employeeId}
                </p>
              </div>
            </div>
        ...
        <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm hover:border-blue-200 hover:shadow-xl transition-all group flex flex-col justify-between">
            <div className="flex justify-between items-start mb-6">
              ...
              <span className="px-3 py-1 bg-blue-50 text-blue-700 text-[10px] font-black rounded-full border border-blue-100">
                Total Dept Clubs
              </span>
            </div>
            <div>
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm hover:border-rose-200 hover:shadow-xl transition-all group flex flex-col justify-between">
            <div className="flex justify-between items-start mb-6">
              ...
              <span className="px-3 py-1 bg-rose-50 text-rose-700 text-[10px] font-black rounded-full border border-rose-100">
                Quality Check
              </span>
            </div>
            <div>
              ...
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                External Event Approvals
              </p>
            </div>
          </div>

          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm hover:border-purple-200 hover:shadow-xl transition-all group flex flex-col justify-between">
            <div className="flex justify-between items-start mb-6 gap-4">
              <div className="p-4 bg-purple-50 text-purple-600 rounded-3xl group-hover:scale-110 transition-transform">
                <Users className="w-8 h-8" />
              </div>
              <div className="flex flex-col items-end gap-2">
                <span className="px-3 py-1 bg-purple-50 text-purple-700 text-[10px] font-black rounded-full border border-purple-100">
                  Selected Event
                </span>
                <select
                  value={selectedEvent?.id ?? ""}
                  onChange={(e) => setSelectedEventId(e.target.value)}
                  className="text-xs font-bold text-slate-700 bg-white border border-slate-200 rounded-xl px-3 py-2 max-w-[220px]"
                >
                  {eventsList.map((evt) => (
            ...
            <div>
              <p className="text-5xl font-black text-slate-900 tracking-tighter mb-1">
                {sectionParticipationCount}
              </p>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                Students Participating (Section{" "}
                {(HOD_PROFILE as any).section ?? "—"})
              </p>
            </div>
          </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-xl font-black text-slate-900 flex items-center gap-3">
              <LayoutDashboard className="w-5 h-5 text-blue-600" /> Recent OD
              Activity
            </h3>
            <button
              onClick={() => setView("fa-od-approvals")}
          ...
          <div className="space-y-4">
            {odRequests.slice(0, 3).map((req) => (
              <div
                key={req.id}
                className="p-5 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-between hover:bg-white hover:shadow-md hover:border-slate-200 transition-all cursor-pointer group"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium">
                      3rd Year • {req.eventName}
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <span className="text-[10px] font-black text-amber-600 uppercase bg-amber-50 px-2 py-1 rounded-lg border border-amber-100">
                    Pending
                  </span>
        ...
        <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
          <div className="flex items-center mb-8">
            <h3 className="text-xl font-black text-slate-900 flex items-center gap-3">
              <BarChart className="w-5 h-5 text-purple-600" /> Dept
              Participation
            </h3>
          </div>
          <div className="space-y-6">
            {[
              { label: "Technical Events", val: "85%" },
              { label: "Workshops", val: "62%" },
              { label: "Cultural / Extra", val: "45%" },
            ].map((item, i) => (
              <div key={i}>
