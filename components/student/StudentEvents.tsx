...
const StudentEvents: React.FC<StudentEventsProps> = ({
  ...
  const filteredEvents = useMemo(() => {
    ...
    if (activeTab === "Internal")
      filtered = filtered.filter((e) => parseInt(e.id) % 2 === 0);
  ...
  const technicalEvents = useMemo(
    () =>
      filteredEvents.filter((e) =>
        ["Workshop", "Hackathon", "Seminar"].includes(e.category)
      ),
    [filteredEvents]
  );
  const nonTechnicalEvents = useMemo(
    () =>
      filteredEvents.filter((e) =>
        ["Cultural", "Sports", "Networking"].includes(e.category)
      ),
    [filteredEvents]
  );
        ...
        <div className="bg-white border border-slate-200 p-1.5 rounded-full flex overflow-x-auto hide-scrollbar shadow-sm">
          {[
            ...
            <button
              ...
              className={`whitespace-nowrap px-6 py-2.5 rounded-full text-[10px] font-black uppercase transition-all ${
                activeTab === t
