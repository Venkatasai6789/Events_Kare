...
const App: React.FC = () => {
  ...
  const handleLogin = (role: 'student' | 'admin' | 'hod') => {
    setUserRole(role);
    setIsLoggedIn(true);
    setView(role === 'admin' ? 'admin-dashboard' : role === 'hod' ? 'hod-dashboard' : 'discover');
  };
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 flex flex-col antialiased selection:bg-blue-100 selection:text-blue-900 font-['Poppins']">
      ...
      <main className="flex-grow w-full max-w-7xl mx-auto px-6 py-8">
        ...
        {userRole === 'student' && currentView === 'events' && (
          <StudentEvents eventsList={eventsList} searchQuery={searchQuery} onEventClick={handleEventClick} activeTab={eventsActiveTab} onTabChange={setEventsActiveTab} />
        )}

        {userRole === 'student' && currentView === 'inbox' && (
          <StudentInbox onUploadClick={() => setShowUploadModal(true)} setPreviewAchievement={setPreviewAchievement} />
        )}
        ...
        {userRole === 'hod' && currentView === 'hod-dashboard' && (
          <HODDashboard setView={setView} odRequests={odRequests} externalProposals={externalProposals} />
        )}
        {userRole === 'hod' && currentView === 'hod-od-approvals' && (
          <HODODApprovals setView={setView} odRequests={odRequests} setOdRequests={setOdRequests} />
        )}
        ...
        {userRole === 'admin' && currentView === 'admin-attendance' && <AdminAttendance />}
        {userRole === 'admin' && currentView === 'admin-certificates' && <AdminCertificates />}
        {userRole === 'admin' && currentView === 'admin-events' && <AdminEvents eventsList={eventsList} setEventsList={setEventsList} />}
        {userRole === 'admin' && currentView === 'admin-vacancies' && <AdminVacancies applications={applications} />}
      </main>
