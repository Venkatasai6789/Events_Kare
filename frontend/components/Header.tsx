...
import { Menu, X, Bell, Search, User, Moon, Sun, Lock, LogOut, ChevronDown, Shield } from 'lucide-react';
...
const Header: React.FC<HeaderProps> = ({ currentView, setView, searchQuery, setSearchQuery, userType, onLogout }) => {
  ...
  const navItems = userType === 'admin' ? adminNavItems : userType === 'hod' ? hodNavItems : studentNavItems;

  const toggleSearch = () => {
    setIsSearchOpen(!isSearchOpen);
    if (!isSearchOpen) {
       if(userType === 'student' && currentView === 'discover') setView('events');
    }
  };
  ...
  return (
    <header 
      className={`sticky top-0 z-50 transition-all duration-300 ${
        scrolled ? 'bg-white/80 backdrop-blur-md border-b border-slate-200/50 shadow-sm' : 'bg-white border-b border-transparent'
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div 
          ...
          onClick={() => setView(userType === 'admin' ? 'admin-dashboard' : userType === 'hod' ? 'hod-dashboard' : 'discover')}
        >
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-lg transition-transform group-hover:scale-105 ${userType === 'admin' || userType === 'hod' ? 'bg-slate-900 shadow-slate-200' : 'bg-blue-600 shadow-blue-200'}`}>
          ...
          <div className="flex flex-col">
            <span className="text-lg font-bold text-slate-900 leading-none tracking-tight">Campus</span>
            <span className={`text-lg font-bold leading-none tracking-tight ${userType === 'admin' || userType === 'hod' ? 'text-slate-600' : 'text-blue-600'}`}>Connect</span>
          </div>
        </div>

        {/* Desktop Nav */}
        <nav className={`hidden md:flex items-center bg-slate-50 px-2 py-1.5 rounded-full border border-slate-200/50 transition-opacity duration-300 ${isSearchOpen ? 'opacity-0 pointer-events-none absolute' : 'opacity-100 relative'}`}>
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => setView(item.id)}
              className={`px-6 py-2 rounded-full text-sm font-semibold transition-all duration-200 ${
                currentView === item.id || (userType === 'student' && currentView === 'event-detail' && item.id === 'events')
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <div className={`flex items-center ${isSearchOpen ? 'bg-slate-100' : ''} rounded-full transition-all duration-300 ease-in-out relative`}>
            <div 
            ...
            >
                <input 
                    ...
                    onChange={(e) => {
                        ...
                        if(currentView !== 'events' && userType === 'student') setView('events');
                        if(userType === 'admin' && currentView !== 'admin-events') setView('admin-events');
                    }}
                    placeholder={userType === 'admin' ? "Search candidates, events..." : "Search events..."}
            ...
            <button 
                ...
                className={`w-10 h-10 flex items-center justify-center rounded-full transition-all ${
                    isSearchOpen ? 'text-slate-900 bg-white shadow-sm' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'
                }`}
            >
          ...
          <div className="relative" ref={profileRef}>
            ...
            >
              <div className={`w-8 h-8 md:w-9 md:h-9 rounded-full flex items-center justify-center text-white shadow-sm ${userType === 'admin' || userType === 'hod' ? 'bg-slate-900' : 'bg-blue-600'}`}>
                {userType === 'admin' || userType === 'hod' ? <Shield className="w-4 h-4" /> : <User className="w-5 h-5" />}
              </div>
            ...
            {isProfileOpen && (
              <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-2xl border border-slate-100 py-3 animate-in fade-in slide-in-from-top-2 duration-200 z-[60]">
                <div className="px-5 py-3 border-b border-slate-50 mb-2">
                  <p className="text-sm font-black text-slate-900 truncate">{userType === 'admin' ? 'Club Admin Access' : userType === 'hod' ? 'Department Head' : 'Alex Thompson'}</p>
                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">{userType === 'admin' ? 'Tech Innovators Guild' : userType === 'hod' ? 'HOD Access' : 'Student Portal'}</p>
                </div>
            ...
            {navItems.map((item) => (
              <button
                ...
                className={`text-left p-4 rounded-xl text-lg font-medium transition-colors ${
                  currentView === item.id ? 'bg-slate-100 text-slate-900' : 'text-slate-600 hover:bg-slate-50'
                }`}
              >
                {item.name}
              </button>
