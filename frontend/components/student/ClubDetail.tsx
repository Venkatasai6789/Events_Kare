...
import { Club, Project, OpenRole, Vacancy } from "../../types";
...
const ClubDetail: React.FC<ClubDetailProps> = ({ club, onBack }) => {
  ...
  const effectiveRoles: Array<OpenRole | (Vacancy & { id: string })> =
    useMemo(() => {
      if (vacancies && vacancies.length > 0) {
        return vacancies.map((v) => ({ ...v, id: v.vacancy_id }));
      }
      return club.roles;
    }, [club.roles, vacancies]);
  ...
  const ProjectCard: React.FC<{ project: Project }> = ({ project }) => (
    <div className="bg-white rounded-[2rem] border border-slate-200 overflow-hidden group hover:shadow-xl transition-all h-full flex flex-col">
      <div className="h-48 overflow-hidden relative">
        ...
        />
        <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-md px-3 py-1 rounded-full border border-slate-100 shadow-sm text-[10px] font-black uppercase tracking-wider text-blue-600">
          {project.badge}
        </div>
      </div>
  ...
  const RoleCard: React.FC<{ role: any }> = ({ role }) => (
    <div className="bg-white border border-slate-200 rounded-[2rem] p-8 hover:border-blue-200 hover:shadow-lg transition-all shadow-sm">
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-sm font-bold text-blue-600">
            {role.openings} Open Position{role.openings > 1 ? "s" : ""}
          </p>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {role.skills.map((skill) => (
            <span
              key={skill}
              className="px-3 py-1 bg-slate-50 text-slate-600 rounded-lg text-[10px] font-black border border-slate-100 uppercase tracking-wide"
            >
              {skill}
            </span>
          ))}
      ...
      <button className="w-full mt-8 py-4 bg-slate-900 text-white rounded-2xl font-bold text-sm hover:bg-slate-800 transition-colors shadow-lg active:scale-95">
        Apply Now
      </button>
    </div>
  );
  ...
  return (
    <div className="animate-in slide-in-from-bottom-8 duration-500">
      <div className="relative h-64 md:h-80 rounded-[2.5rem] overflow-hidden mb-8 group">
        ...
        <button
          onClick={onBack}
          className="absolute top-8 left-8 bg-white/20 backdrop-blur-md text-white p-3 rounded-full hover:bg-white hover:text-slate-900 transition-all shadow-lg border border-white/20"
        >
          ...
          <section>
            ...
            {loadingVacancies && (
              <p className="text-slate-500 font-bold mb-4">
                Loading latest openings...
              </p>
            )}
            <div className="grid gap-6">
              {effectiveRoles.map((r: any) => (
