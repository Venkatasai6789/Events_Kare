...
const StudentEvents: React.FC<StudentEventsProps> = ({ eventsList, searchQuery, onEventClick, activeTab, onTabChange }) => {
  ...
  const filteredEvents = useMemo(() => {
    ...
    if (activeTab === 'Internal') filtered = filtered.filter(e => e.type !== 'External');
    else if (activeTab === 'External') filtered = filtered.filter(e => e.type === 'External');
    else if (activeTab === 'Registered') filtered = filtered.filter(e => e.isRegistered);

    return filtered;
  }, [activeTab, searchQuery, eventsList]);

  const technicalEvents = useMemo(() => filteredEvents.filter(e =>
    ['Workshop', 'Hackathon', 'Seminar', 'Faculty Development Program', 'Internship',
      'Short Term Course', 'Conference', 'Symposium', 'Guest Lecture', 'Technical Fest',
      'Webinar', 'Online Course', 'Project Contest', 'Paper Presentation', 'Value Added Course', 'Training', 'Events'].includes(e.category) ||
    (!['Cultural', 'Sports', 'Networking', 'Cultural Fest', 'Management Fest', 'Sports Event'].includes(e.category) && e.type === 'External')
  ), [filteredEvents]);

  const nonTechnicalEvents = useMemo(() => filteredEvents.filter(e =>
    ['Cultural', 'Sports', 'Networking', 'Cultural Fest', 'Management Fest', 'Sports Event'].includes(e.category)
  ), [filteredEvents]);
        ...
        <div className="bg-white border border-slate-200 p-1.5 rounded-full flex overflow-x-auto hide-scrollbar shadow-sm">
          {['All', 'Internal', 'External', 'Registered'].map(t => (
            <button
              ...
              className={`whitespace-nowrap px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-wider transition-all ${activeTab === t ? 'bg-slate-100 text-slate-900' : 'text-slate-400 hover:text-slate-600'}`}
            >
