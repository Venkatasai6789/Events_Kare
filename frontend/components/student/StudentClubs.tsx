...
import { Club, Vacancy } from "../../types";
...
const StudentClubs: React.FC<StudentClubsProps> = ({ onClubClick }) => {
  ...
  const [clubs, setClubs] = useState<Club[]>(CAMPUS_CLUBS);
  ...
  useEffect(() => {
    const loadCounts = async () => {
      ...
      try {
        ...
        const vacancies: Vacancy[] = Array.isArray(data?.vacancies)
          ? data.vacancies
          : [];
        const countsByClub = new Map<string, number>();
        for (const v of vacancies) {
          if (!v?.club_name || !clubNameSet.has(v.club_name)) continue;
          const prev = countsByClub.get(v.club_name) || 0;
          countsByClub.set(v.club_name, prev + (Number(v.openings) || 0));
        }
  ...
  const ClubCard: React.FC<{ club: Club }> = ({ club }) => (
    <div className="bg-white rounded-[2rem] border border-slate-200 p-8 flex flex-col md:flex-row gap-8 items-center md:items-start group hover:shadow-2xl hover:shadow-slate-200/50 hover:border-blue-200 transition-all duration-300">
      ...
      <div className="flex-grow flex flex-col justify-center text-center md:text-left">
        ...
        <p className="text-slate-500 font-medium mb-6 line-clamp-2 max-w-2xl leading-relaxed">
          {club.description}
        </p>
        <div className="flex items-center justify-center md:justify-start gap-6 mb-6">
          <div className="flex items-center gap-2 text-slate-500 text-sm font-bold">
            <Users className="w-4 h-4 text-blue-500" /> {club.members} Members
          </div>
          <div className="flex items-center gap-2 text-slate-500 text-sm font-bold">
            <Target className="w-4 h-4 text-rose-500" /> {club.openPositions}{" "}
            Open Positions
          </div>
        </div>
        <div className="flex flex-wrap justify-center md:justify-start gap-2">
          {club.tags.map((tag) => (
            <span
              key={tag}
              className="px-3 py-1 bg-slate-50 text-slate-500 rounded-full text-[10px] font-black uppercase tracking-wider border border-slate-100"
            >
              {tag}
            </span>
          ))}
      ...
      <div className="flex-shrink-0 md:self-center">
        <button
          ...
          className="px-8 py-4 bg-slate-900 text-white rounded-2xl font-bold text-sm flex items-center gap-2 hover:bg-blue-600 transition-all active:scale-95 shadow-md shadow-slate-200"
        >
    ...
    <div className="animate-in fade-in duration-500 pb-20">
      <div className="mb-10">
        ...
        <p className="text-slate-500 font-medium mt-2">
          Join a community, build projects, and accelerate your career.
        </p>
      </div>
      {loadingCounts && (
        <p className="text-slate-500 font-bold mb-6">Refreshing openings...</p>
      )}
