...
const EventCard: React.FC<EventCardProps> = ({ event, compact = false, onClick }) => {
  ...
  return (
    ...
    >
      {/* Image Area */}
      <div className={`${compact ? 'h-40' : 'h-48'} relative overflow-hidden bg-slate-100`}>
        {event.image ? (
          <img
            src={event.image}
            alt={event.title}
        ...
        <div className="absolute top-3 left-3 bg-white/95 backdrop-blur-md rounded-xl p-2 text-center min-w-[50px] shadow-sm ring-1 ring-black/5">
          <p className="text-[10px] font-bold text-red-500 uppercase tracking-wider mb-0.5">{month}</p>
          <p className="text-lg font-black text-slate-900 leading-none">{day}</p>
        </div>

        {/* Status Badge */}
        <div className="absolute top-3 right-3 flex flex-col gap-2 items-end">
          <span className={`px-2.5 py-1 rounded-lg backdrop-blur-md border border-white/10 text-white text-[10px] font-bold uppercase tracking-wider shadow-sm ${event.status === 'Ongoing' ? 'bg-emerald-500/90' :
        ...
        <div className="absolute bottom-3 right-3">
          {(event.feeShort === 'Free' || event.registrationFees === 'Free') ? (
            <span className="px-3 py-1 rounded-lg bg-emerald-500 text-white text-[10px] font-bold shadow-lg tracking-wide ring-1 ring-white/20">FREE</span>
          ) : (
      ...
      <div className="p-5 flex flex-col flex-grow relative">
        ...
        <div className="mb-4">
          <div className="flex items-center gap-1.5 mb-2 text-slate-500">
            ...
            <p className="text-[10px] font-bold uppercase tracking-wide truncate">{event.organizer || "Unknown Organizer"}</p>
          </div>
          <h3 className={`font-bold text-slate-800 leading-snug group-hover:text-blue-600 transition-colors line-clamp-2 ${compact ? 'text-base' : 'text-lg'}`}>
            {event.title}
          </h3>
        </div>
